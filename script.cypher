...
CREATE (post:Post {post_id: 7});
CREATE (post:Post {post_id: 8});
CREATE (post:Post {post_id: 9});
CREATE (post:Post {post_id: 10});
CREATE (post:Post {post_id: 11});

// Associação dos posts com os usuários

MATCH (user), (post)
WHERE user.account_id = 1 AND post.post_id = 1
CREATE (user)-[:PUBLISH {datetime: datetime()}]->(post);
...
MATCH (user), (post)
WHERE user.account_id = 7 AND post.post_id >= 7
CREATE (user)-[:PUBLISH {datetime: datetime()}]->(post);
...
MATCH (user1), (user2)
WHERE user1.account_id = 3 AND user2.account_id = 6
CREATE (user1)-[:FOLLOW {datetime: datetime()}]->(user2);
...
MATCH (post1), (post2)
WHERE post1.post_id = 2 AND post2.post_id = 1
CREATE (post1)-[:SHARE {datetime: datetime()}]->(post2);

MATCH (post1), (post2)
WHERE post1.post_id = 7 AND post2.post_id = 2
CREATE (post1)-[:SHARE {datetime: datetime()}]->(post2);

// Associação entre dois posts (resposta)

MATCH (post1), (post2)
WHERE post1.post_id = 5 AND post2.post_id = 1
...
MATCH path = ((user1:User {account_id: 1}) - [:FOLLOW *] - (:User {account_id: 6}))
...
WITH post as post, collect(distinct user2) as c1
...
// Usuários que publicam mais que N = 5 valores em um intervalo M = 24 de horas.
WITH 5 as N, duration({hours: 24}) as M
